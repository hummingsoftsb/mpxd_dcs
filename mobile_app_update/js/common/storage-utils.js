(function(){
  ...
  function LocalForageUtils($localForage, $q, $log, Utils, Config){
    ...
    function _get(key, _defaultValue){
      if(storageCache[key]){
        ...
        if(Config.storage){
          promiseStorageCache[key] = $localForage.getItem(finalKey).then(function(value){
            try {
  ...
  function LocalStorageProvider(Config){
    ...
    function _get(key, _defaultValue, dontUsePrefix){
		dontUsePrefix = (typeof dontUsePrefix == 'boolean') && dontUsePrefix;
		var finalKey = dontUsePrefix ? key : Config.storagePrefix+key;

      if(!storageCache[key]){
        if(Config.storage && window.localStorage){
          try {
            storageCache[key] = JSON.parse(window.localStorage.getItem(finalKey)) || angular.copy(_defaultValue);
          } catch(e) {
            storageCache[key] = angular.copy(_defaultValue);
          }
        } else {
    ...
    function LocalStorageUtils($window, $log, Utils){
      var service = {
        get:                function(key, _defaultValue, dontUsePrefix)  { return Utils.async(function(){return _get(key, _defaultValue, dontUsePrefix);});         },
