...
Class Assessment extends CI_Model
{
    ...
    function show_pending_journal_dataentry_p($data,$offset,$perPage,$userid,$roleid)
    {
        $data=strtolower($data);
        $data=str_replace("'","''",$data);
        $query ="select a.project_no,a.project_name,b.journal_name,b.journal_no,c.data_entry_status_id,d.frequency_detail_name,e.user_full_name from project_template a, journal_master b, journal_data_entry_master c, frequency_detail d, sec_user e, journal_data_user f where a.project_no=b.project_no and b.journal_no=c.journal_no and c.data_entry_status_id=1 and b.journal_no=f.journal_no and c.frequency_detail_no=d.frequency_detail_no and e.user_id = f.data_user_id";
        if($userid!="1" && $roleid!="1")
        {
