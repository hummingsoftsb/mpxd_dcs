...
<script>
	$(document).ready(function()
	{
		...
		{
			save_data(function(){window.location.reload()});
			return false;
   		});	
		...
		$('.image-description .save').on("click", function(){
			...
			if(datadesc.length > 1){
				$.post( "<?php echo base_url(); ?><?php echo $cpagename; ?>/updateimagedesc",{picid:dataid,imagedesc1:datadesc}, function( data ){
					var status=data.st;
					if(status === 1){
		...
		$("#tableimage tbody").sortable({
			...
			stop: function(event,ui) {renumber_table('#tableimage')}
		}).disableSelection();
...
<!--	 --><?php //var_dump($message_type); ?><!-- -->
	...
	<?php echo $is_image == 1 ? '<input type="hidden" name="isimage" value="1"/>' : '' ?>
		<fieldset style="<?php echo $is_image == 1 ? 'display: none;' : '' ?>">
			...
			<table class="table table-striped table-hover ">
				...
				<tbody>
					<style type="text/css">.highlight_error{background: #ff5;}.alertstriphidden{visibility: hidden;}</style>
					<?php
						$sno=1;
		...
		<fieldset style="<?php echo $is_image == 1 ? '' : 'display: none;' ?>">
			...
			<p style="text-align: right;"><?php echo $labelname[15]; ?> &nbsp &nbsp &nbsp <a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg2"><button type="button" class="btn btn-success btn-sm pull-right" data-toggle="modal" data-target="#myModal" id="modaladd" name="modaladd">Upload</button></a></p>
			<small><?php echo $reject_note != '' ? '<b class="text-default">'.$labelname[13].': </b>'. $reject_note .'': '' ?></small>
			<?php 	
						...
						<tbody>
							<?php
								foreach($dataimages as $dataimage):
									...
									//echo '<td><a href="#" data-toggle="modal" class="modaledit" data-target="#MyModal1" data-picid="'.$dataimage->data_entry_pict_no.'" data-img="'.$dataimage->pict_file_path.$dataimage->pict_file_name.'" data-desc="'.$dataimage->pict_definition.'"><span class="glyphicon glyphicon-edit">&nbsp;</span></a></td>';
		...
		<div class="form-group" style="text-align: center;">
			<a href="javascript:void(0)" class="btn btn-success btn-sm" id="modalpublish" onclick="showloader();">Publish</a> &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp
	  	...
	  	<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
			  	...
			  	<div class="modal-body">
			  		<form method=post id=updateimage action="<?php echo base_url(); ?><?php echo $cpagename; ?>/updateimage/" enctype="multipart/form-data">
						<div class="form-group">
							<div class="col-xs-4" style="text-align: right;"></div>
						...
						<br/>
						<div class="form-group">
							<div class="col-xs-4" style="padding-left: 210px; text-align: right;"><label for="exampleInputFile" class="col-lg-3 control-label"><?php echo $labelname[18]; ?></label></div>
				  		...
				  		<br>
					  	<div class="form-group">
							<div class="col-xs-4" style="text-align: right;"><label for="exampleInputEmail1" class="control-label"><?php echo $labelname[17]; ?></label></div>
	...
	<!-- -------------------------------------------- -->
	<!-- pop-up -->
	<div class="modal fade bs-example-modal-lg3" id="MyModal2" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
	  	<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
			    	<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">?</span><span class="sr-only">Close</span></button>
			  			...
			  			<table class="table table-striped table-hover " id="varient">
							<thead>
								<tr>
									...
									<th><?php echo $labelname[26]; ?><input type="hidden" id="varientcount" name="varientcount" /></th>
								</tr>
							</thead>
							<tfoot>
    							<tr>
    								<td colspan="10" align="center"><input type="submit" class="btn btn-primary btn-sm" value="Save" onclick="showloader();" />&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-sm" data-dismiss="modal">Cancel</button></td>
    							</tr>
							</tfoot>
						</table>
				  	</form>
	...
	$.each(items, function(idx,i) {
		...
		if (i.type == "1") {
			...
			for (var j = 0; j < i.dependOn.length; j++) {
				if (!items[i.dependOn[j]].reachedMax) { enable = false; break; }
			}
			if (enable) {
