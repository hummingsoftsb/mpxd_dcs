...
class Journalvalidation extends CI_Controller
{
	...
	function index($offset=0)
	{
		...
		if($this->session->userdata('logged_in'))
		{
			...
			if($this->uri->uri_string()=="journalvalidation" && $_SERVER['QUERY_STRING']=="")
			{
			...
			$config['base_url'] = base_url().'index.php/journalvalidation/index';
			...
			$data['records'] = $this->assessment->show_valde($search,$offset,$config['per_page'],$userid,$roleid);
			$data['totalrows'] = $config['total_rows'];
			$data['mpage'] = $config['per_page'];
			$data['page']= $page+1;
			$data['selectrecord']=$config['per_page'];
			$data['searchrecord']=$search;
			$data['cpagename']='journalvalidation';
			...
			$this->load->view('assess_journalvalidation', $data);
	...
	{
		$sess_array = array('searchrecord' => $this->input->post('search')); 
