...
class Securityuser extends CI_Controller
{
	...
	function add()
	{
		...
		else
		{
			...
			if($this->securitys->add_check_user($username,$email)==0)
			{
				...
				//send email
				$this->swiftmailer->add_new_user($fullname, $username, $email, $pass);
				//query the database
				...
				$this->email->to($email);
			    $message="Dear ".$fullname.", <br>Your login detail are as follows:<br><br><b>URL</b> : ".$this->config->item('base_url2')."<br><b>Username</b> : ".$username."<br><b>Email</b> : ".$email."<br><b>Password</b> : ".$pass;
				...
				*/
				...
				$sess_array = array('message' =>  $this->securitys->get_label_object(14)." Added Successfully","type" => 1);
	...
	function update()
	{
		...
		else
		{
			...
			if($this->securitys->update_check_user($id,$username,$emailid)==0)
			{
				...
				$sess_array = array('message' => $this->securitys->get_label_object(14)."  Updated Successfully","type" => 1);
				$this->session->set_userdata('message', $sess_array);
	...
	function reset()
	{
		...
		$this->load->library('form_validation');
		$this->form_validation->set_rules('pass', $label, 'trim|required|xss_clean|alpha_numeric_spaces_special|matches[cpass]');
		$this->form_validation->set_rules('cpass', $label1, 'trim|required|alpha_numeric_spaces_special|xss_clean');
		...
		else
		{
			...
			//send email
			$user_data = $this->securitys->get_user_by_id($id);
			$this->swiftmailer->reset_password($user_data->user_full_name, $user_data->user_name, $user_data->email_id, $this->input->post('pass'));
			...
			$sess_array = array('message' => $label." reset successfully","type" => 1);
			$this->session->set_userdata('message', $sess_array);
			echo json_encode(array('st'=>1, 'msg' => 'Success'));
		}
	}
	...
	function delete()
	{
		...
		{
			$sess_array = array('message' => "Cannot delete ".$this->securitys->get_label_object(14).", Assigned to ".$this->securitys->get_label_object(6),"type" => 0);
			$this->session->set_userdata('message', $sess_array);
		}
	}
