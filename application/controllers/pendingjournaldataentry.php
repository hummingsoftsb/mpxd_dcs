...
class Pendingjournaldataentry extends CI_Controller
{
    ...
    function index($offset = 0)
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $roleperms = $this->securitys->show_permission_object_data($roleid, "26");
            ...
            if ($this->uri->uri_string() == "pendingjournaldataentry" && $_SERVER['QUERY_STRING'] == "") {
            ...
            $data['cpagename'] = 'pendingjournaldataentry';
            $data['labels'] = $this->securitys->get_label(3);
            $data['labelgroup'] = $this->securitys->get_label_group(26);
            $data['labelobject'] = $this->securitys->get_label_object(26);
    ...
    private function journal_nonp($offset = 0)
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            // Config setup for Pagination
            $config['base_url'] = base_url() . 'index.php/pendingjournaldataentry/index';
            $config['total_rows'] = sizeOf($this->ilyasmodel->get_journals_nonp_pending($search, 0, null, $userid, false, $roleid));
            if ($this->session->userdata('selectrecord')) {
                $selectrecord = $this->session->userdata('selectrecord');
                $config['per_page'] = $selectrecord['selectrecord'];
            } else {
            ...
            $data['records'] = $this->ilyasmodel->get_journals_nonp_pending($search, $offset, $config['per_page'], $userid, false, $roleid);
            $data['totalrows'] = $config['total_rows'];
            $data['mpage'] = $config['per_page'];
            $data['page'] = $page + 1;
            $data['selectrecord'] = $config['per_page'];
